    ...
    def _query(self, node: IntervalNode, qx1: float, qx2: float, qy1: float, qy2: float) -> list:
        ...
        if is_in_range(qx1, qx2, node.x_median):
            for i in node.intervals:
                if is_in_range(qx1, qx2, i[0]):
                    if is_in_range(qy1, qy2, i[1]):
                        matching_intervals.append(i)

            left_node = node.left_child
            right_node = node.right_child

            matching_intervals += self._query(left_node, qx1, qx2, qy1, qy2)
            matching_intervals += self._query(right_node, qx1, qx2, qy1, qy2)
        else:
            return []
