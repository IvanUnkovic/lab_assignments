...
class MNISTMetricDataset(Dataset):
    ...
    def _sample_negative(self, index):
        ...
        negative_class = choice([cls for cls in self.classes if cls != target_id and len(self.target2indices[cls]) > 0])
        return choice(self.target2indices[negative_class])

    def _sample_positive(self, index):
        target_id = self.targets[index].item()
        positive = self.target2indices[target_id]
        positive = [idx for idx in positive if idx != index]
